...
""")
dict_for_db = {}
print(type(dict_for_db))

# function for downloading from VK
...
# function for downloading info from user page
def page_download(page_id):
    ...
    page_info = vk_download('users.get', 'user_ids=' + page_id + parameters_for_download)
    page_info = page_info[0]
    page_name = page_info['first_name'] + ' ' + page_info['last_name']
    try:
        mobile_phone = page_info.get('mobile_phone')
        if mobile_phone == "":
            mobile_phone = "None"
    except ValueError:
        mobile_phone = "None"
    try:
        site = page_info.get('site')
        if site == "":
            site = "None"
    except ValueError:
        site = "None"
        ...
        if int(download_note['from_id']) == int(download_note['owner_id']):
            ...
            if count_likes != 0:
                for j in range(0, count_likes):
                    ...
                    param_post += 'extended=1&offset=' +str(j) +'&count=1&v=5.81'
                    post = vk_download('likes.getList', param_post)
                    post = post['items']
                    post = post[0]
                    id_page = str(post['id'])
                    spis = []
                    if not (dict_for_db.get(id_page)):
                        page = page_download(id_page)
                        like = 1
                        spis = [page, like]
                        dict_for_db[id_page] = spis
                    else:
                        spis = dict_for_db[id_page]
                        spis[1] += 1
                        dict_for_db = spis
                    time.sleep(0.5)
                k += 1
...
dkeys = dict_for_db.keys()
for dkey in dkeys:
    spis = dict_for_db[dkey]
    id_page = spis[0][1]
    name = spis[0][0]
    phone = spis[0][2]
    site = spis[0][3]
